...
为了让苹果地图和高德走同一条流程，链接统一发给 `wloc-spoofer.bingyang-hou.workers.dev/api/parse` 解析：
...
- 部署后把快捷指令里的 `wloc-spoofer.bingyang-hou.workers.dev` 换成你自己的 worker 域名即可。
...
- **Workers**: `https://wloc-spoofer.bingyang-hou.workers.dev/`
